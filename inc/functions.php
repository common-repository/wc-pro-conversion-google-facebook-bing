...
function get_conversion_code(){
	...
	if( is_order_received_page() ){
		...
		if(!empty($thankyou_script)) return $thankyou_script;		
	}
		...
		if(count( $product_script ) == 0){
			...
			if( $terms ){
				...
				foreach ($terms as $term):
					...
					$cat_script = get_woocommerce_term_meta( $product_cat_id, $key);
					
					if(!$cat_script) continue;
	...
	if( is_product_category() ){
		...
		$cat_script = get_woocommerce_term_meta( $cat_id, $key ); 				
				
		if( $cat_script ) return $cat_script; // return code of current category
		
		if( !$cat_script ){
			if($cat->parent > 0){ // if category has parent category
	
				// check code of parent category
	
				$cat_script = get_woocommerce_term_meta( $parent_id, $key);
...
function display_conversion_code(){
	...
	if( is_assoc($scripts) ){
		...
		foreach( $scripts as $script ):
			...
			if (isset($script['_cn_gcode'])) $gcode = $script['_cn_gcode'];

			if (isset($script['_cn_ycode'])) $ycode = $script['_cn_ycode'];

			if (isset($script['_cn_bcode'])) $bcode = $script['_cn_bcode'];

		endforeach;

	}

	echo "<!-- Conversion Code -->\n";

	if(!empty($gcode)) echo strip_slashes_script($gcode)."\n\n";

	if(!empty($ycode)) echo strip_slashes_script($ycode)."\n";

	if(!empty($bcode)) echo strip_slashes_script($bcode)."\n";

	echo "<!-- EOF Converstion Code -->\n";

		

}

add_action('wp_head', 'display_conversion_code', 9);
...
function strip_slashes_script( $code ){

	return stripslashes_deep( $code );
